...
if [[ -d "$HOME/.nvm/versions/node" ]]; then
  nvm_latest_dir=$(find "$HOME/.nvm/versions/node" -mindepth 1 -maxdepth 1 -type d -printf '%f\n' 2>/dev/null | sort -V | tail -1)
  if [[ -n "$nvm_latest_dir" ]]; then
    nvm_default_npx="$HOME/.nvm/versions/node/$nvm_latest_dir/bin/npx"
  fi
fi
